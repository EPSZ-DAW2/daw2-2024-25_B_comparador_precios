...
class TiendasController extends Controller
{
	...
	public function actionAdminTienda()
	{
		...
		// Verificar si el usuario tiene rol 'Usuario Tienda' o 'Superadministrador'
		if (Usuario::tieneRol($usuarioId, Usuario::ROL_USUARIO_TIENDA) || 
			Usuario::tieneRol($usuarioId, Usuario::ROL_SUPERADMINISTRADOR)) {
	...
	{
		if (Yii::$app->user->isGuest) {
			Yii::$app->session->setFlash('error', 'Debes iniciar sesión para denunciar una tienda.');
			return $this->redirect(['site/login']);
		}
	...
	{
		if (Yii::$app->user->isGuest) {
			Yii::$app->session->setFlash('error', 'Debes iniciar sesión para seguir una tienda.');
			return $this->redirect(['site/login']);
		}
