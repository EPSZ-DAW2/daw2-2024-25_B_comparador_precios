...
class RegistrosController extends Controller
{
    public function actionRegister()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if ($model->save()) {
                ...
                if ($aviso->save()) {
                    $this->actionLog('Registro', 'El usuario se registró exitosamente', 'Registro de Usuario');
    ...
    // Función protegida para los registros de logs
    protected function actionLog($accion, $detalles, $tipoAccion)
    {
        $log = new RegistroLogs();
        $log->usuario = Yii::$app->user->isGuest ? 'Invitado' : Yii::$app->user->identity->username; // Si no está autenticado, es un invitado
        $log->nivel = 'INFO'; // Nivel de log
        $log->mensaje = $detalles;
        $log->accion = $tipoAccion; // Nuevo campo acción
