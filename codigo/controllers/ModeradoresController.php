...
class ModeradoresController extends Controller
{
     ...
     */
    public function actionCrearComentarioTienda($articuloId, $tiendaId)
    {
        // Crear una nueva instancia del modelo Comentario
        $comentario = new Comentario();
        
        // Establecer los valores de articulo_id y tienda_id
        ...
        if ($comentario->load(Yii::$app->request->post()) && $comentario->validate()) {
            ...
            if ($comentario->save()) {
                ...
                // Recargar el articuloTienda para obtener los comentarios actualizados
                $articuloTienda = ArticulosTienda::findOne(['articulo_id' => $articuloId, 'tienda_id' => $tiendaId]);
                if (!$articuloTienda) {
                    throw new NotFoundHttpException('No se encontró el artículo-tienda.');
                }

                // Crear el ActiveDataProvider para los comentarios relacionados
                $comentariosQuery = $articuloTienda->getComentarios(); // Relación de comentarios
                $comentariosDataProvider = new \yii\data\ActiveDataProvider([
                    'query' => $comentariosQuery,
                    'pagination' => [
                        'pageSize' => 10, // Ajusta el tamaño de página si es necesario
                    ],
                ]);

                // Redirigir a la vista con los comentarios actualizados
                return $this->render('ver-articulo-tienda', [
                    'articuloTienda' => $articuloTienda,
                    'comentariosDataProvider' => $comentariosDataProvider,
                ]);
