    ...
    */
    ...
    public function actionCrearTienda()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            // Si los campos están vacíos, asignarlos como NULL
            if (empty($model->registro_id)) {
                $model->registro_id = null;
            }

            if (empty($model->seguimiento_id)) {
                $model->seguimiento_id = null;
            }
    ...
    /**
     * Edita el articulo de una tienda
     */
    ...
    /**
     * Elimina el articulo de una tienda
     */
    ...
    public function actionEditarAviso($id)
    {
        ...
        // Validar que el aviso tenga un usuario_destino_id asignado
        if (!$aviso->usuario_destino_id) {
            throw new \yii\web\NotFoundHttpException('El aviso no tiene un usuario destino asignado.');
        }

        // Procesar el formulario
        if ($aviso->load(Yii::$app->request->post())) {
            // Intentar guardar el modelo con los datos editados
            if ($aviso->save()) {
                ...
                Yii::$app->session->setFlash('error', 'No se pudo actualizar el aviso: ' . json_encode($aviso->getErrors()));
            }
        }
    ...
    {
        if (($aviso = Aviso::findOne($id)) !== null) {
            return $aviso;
        }

        throw new \yii\web\NotFoundHttpException('El aviso no existe.');
    }
